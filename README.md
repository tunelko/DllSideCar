...
Each push to `develop`, `main`, or `release/**` triggers four jobs in
GitHub Actions:

1. **Build and test** on `windows-latest` against .NET 9.
2. **NuGet vulnerability audit** via `dotnet list package
   --vulnerable --include-transitive`. Fails on any vulnerable direct
   or transitive dependency.
3. **Static analysis** with the .NET Roslyn analyzer set plus
   `SecurityCodeScan.VS2019`. The build is configured to fail on any
   diagnostic in the `Security` analyzer category (CA21xx, CA31xx,
   CA53xx, SCS00xx).
4. **Secret scan** with `gitleaks`.

Dependabot watches both `nuget` and `github-actions` ecosystems and
...
| Branch            | Purpose                                                 |
|-------------------|---------------------------------------------------------|
| `develop`         | Default integration branch. Day-to-day commits.         |
| `release/X.Y.Z`   | Cut from `develop` to stabilise a release.              |
| `main`            | Receives only `release/*` merges. Ruleset-protected: pull request required, all status checks required, linear history, no force push. |

## License

Research and personal use. The tool generates source code and binaries
intended for authorised security research. Use against systems you do
not own or do not have explicit written permission to test is
prohibited.

## Author

Pedro J. Nunez-Cacho Fuentes — independent security researcher.

- Handle: [@tunelko](https://github.com/tunelko)
- Blog: [blogs.tunelko.com](https://blogs.tunelko.com)
