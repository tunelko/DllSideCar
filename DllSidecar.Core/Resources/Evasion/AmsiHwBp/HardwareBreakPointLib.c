...
#include "Structs.h"
#include "HardwareBreakPointLib.h"
...
// Sets or Removes a hardware breaking point							\
	...
	* Init		= Set(TRUE)/Remove(FALSE) a hardware breaking point	
BOOL SetHardwareBreakpoint(IN DWORD ThreadId, IN PUINT_VAR_T Address, IN DRX Drx, IN BOOL bInitializeHWBP);


// Veh handler function that handles the hardware breaking points exception
LONG ExceptionHandlerCallbackRoutine(IN PEXCEPTION_POINTERS ExceptionInfo);


// Used to set/remove hardware breaking points among all running threads					\
	* Address			= Harware breaking point address (where to install)					\
	* Position			= Can be 0 -> 3, represnting Dr0-Dr3 registers						\
	* Init				= Set(TRUE)/Remove(FALSE) a hardware breaking point					\
	* Tid				= Thread Identifier to hook	| 0 to hook all threads					
...
BOOL CleapUpHardwareBreakpointHooking() {
	...
	// Remove all installed breaking points
...
LONG ExceptionHandlerCallbackRoutine(IN PEXCEPTION_POINTERS ExceptionInfo)
{
	...
	while (TempObject != NULL) {
		...
		// Search for the detour function pointer of the breaking point triggered if the node is not processed yet
		if (TempObject->Address == ExceptionInfo->ContextRecord->Rip && !TempObject->Processed) {
			...
			// 3. enable the hw breaking point again
...
BOOL SetHardwareBreakpoint(IN DWORD ThreadId, IN PUINT_VAR_T Address, IN DRX Drx, IN BOOL bInitializeHWBP)
{
	...
	if (bInitializeHWBP) {
		...
		// Enable the breaking point: Populate the G0-3 flags depending on the saved breaking point position in the Dr0-3 registers
		Context.Dr7 = SetDr7Bits(Context.Dr7, (Drx * 2), 1, 1);
	}
	// If remove
	else {
		// If breaking point found in the thread context
		if ((&Context.Dr0)[Drx] == Address) {
			// Remove the address of the hooked function from the thread context
			(&Context.Dr0)[Drx] = 0ull;
			// Disabling the hardware breaking point by setting the target G0-3 flag to zero
