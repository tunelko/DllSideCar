...
public static class TemplateEngine
{
    ...
    /// <summary>
    /// Renders the trailing " — &lt;handle&gt;" attribution for header comments
    /// only when the researcher field is non-empty. Distributed installer ships
    /// with TemplateConfig.Researcher = "" so a fresh-machine PoC gets a clean
    /// "/* Generated by DllSidecar */" header without an em-dash dangling.
    /// </summary>
    ...
    // File-scope multi-byte XOR key emission. Called once per generated .c when
    // EncryptStrings is on; every xor_decrypt_to callsite in the same file
    // references g_xkey by name. Rotating multi-byte key defeats simple
